 ...
 * @return array
 */
function ESGI_Custom_body_classes( $classes ) {
...
add_filter( 'body_class', 'ESGI_Custom_body_classes' );
 ...
 * @return array
 */
function ESGI_Custom_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'ESGI_Custom_post_classes', 10, 3 );
 ...
 * @return void
 */
function ESGI_Custom_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'ESGI_Custom_pingback_header' );
 ...
 * @return void
 */
function ESGI_Custom_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'ESGI_Custom_supports_js' );
 ...
 * @return array
 */
function ESGI_Custom_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'ESGI_Custom_comment_form_defaults' );
 ...
 * @return bool
 */
function ESGI_Custom_can_show_post_thumbnail() {
	/**
 ...
 * @return int
 */
function ESGI_Custom_get_avatar_size() {
	return 60;
}
 ...
 * @since Twenty Twenty-One 1.0
 */
function ESGI_Custom_continue_reading_text() {
 ...
 * @since Twenty Twenty-One 1.0
 */
function ESGI_Custom_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . ESGI_Custom_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'ESGI_Custom_continue_reading_link_excerpt' );
 ...
 * @since Twenty Twenty-One 1.0
 */
function ESGI_Custom_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . ESGI_Custom_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'ESGI_Custom_continue_reading_link' );

if ( ! function_exists( 'ESGI_Custom_post_title' ) ) {
	/**
 ...
 * @return string
 */
function ESGI_Custom_get_icon_svg( $group, $icon, $size = 24 ) {
	return ESGI_Custom_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function ESGI_Custom_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? ESGI_Custom_get_icon_svg( 'ui', 'arrow_right' ) : ESGI_Custom_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? ESGI_Custom_get_icon_svg( 'ui', 'arrow_left' ) : ESGI_Custom_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'ESGI_Custom_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function ESGI_Custom_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'ESGI_Custom_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'ESGI_Custom_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'ESGI_Custom_generate_css' ) ) {
	...
	return ESGI_Custom_generate_css( // @phpstan-ignore-line.
 ...
 */
function ESGI_Custom_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function ESGI_Custom_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
...
add_filter( 'the_password_form', 'ESGI_Custom_password_form', 10, 2 );
 ...
 */
function ESGI_Custom_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'ESGI_Custom_get_attachment_image_attributes', 10, 3 );
